                        ...
                        <div class="record-header">
                            <div class="records table-responsive">
                                <table id="example" class="table table-striped" style="width: 100%; text-align: center; vertical-align: middle;">
                                    ...
                                    <tbody>
                                        <?php
                                        $sql = "SELECT * FROM `tbl_bus_management`";
                                        $result = mysqli_query($link, $sql);
                                        while ($row = mysqli_fetch_assoc($result)) {
                                        ?>
                                            <tr>
                                                <td style="text-align: center; vertical-align: middle;"><?php echo $row["id"] ?></td>
                                                <td style="text-align: center; vertical-align: middle;"><?php echo $row["busno"] ?></td>
                                                <td style="text-align: center; vertical-align: middle;"><?php echo $row["type"] ?></td>
                                                <td style="text-align: center; vertical-align: middle;"><?php echo $row["purchdate"] ?></td>
                                                <td style="text-align: center; vertical-align: middle;"><?php echo $row["operator"] ?></td>
                                                <td style="text-align: center; vertical-align: middle;"><?php echo $row["driver"] ?></td>
                                                <td style="text-align: center; vertical-align: middle;"><?php echo $row["route"] ?></td>
                                                <td style="text-align: center; vertical-align: middle;">
                                                    <?php
                                                    $status = $row["status"];
                                                    if ($status == "in-service") {
                                                        echo "<span class='badge badge-lg badge-success text-white'>In Service</span>";
                                                    } elseif ($status == "maintenance") {
                                                        echo "<span class='badge badge-lg badge-info text-white'>Maintenance</span>";
                                                    } elseif ($status == "not available") {
                                                        echo "<span class='badge badge-lg badge-dark text-white'>Not Available</span>";
                                                    } else {
                                                        echo $status;
                                                    }
                                                    ?>
